...
const initializeWorkspaceIntegration = () => {
  ...
  // Chatbot 主动拉取最新选区：收到请求后立即广播当前选区
  const handleGetCurrentSelection = () => {
    try { notifySelectionChanged(); } catch {}
  };
  window.addEventListener('get-current-selection', handleGetCurrentSelection);
  ...
  onBeforeUnmount(() => {
    ...
    window.removeEventListener('get-current-selection', handleGetCurrentSelection);
...
const applySmartEditToMonaco = (action: string, scope: string, result: string, originalText: string) => {
  ...
  try {
    ...
    } else if (scope === 'selection' || scope.includes('段落') || scope.includes('标题') || scope.includes('列表')) {
      // 尝试智能定位到相关内容并替换
      const content = model.getValue();
