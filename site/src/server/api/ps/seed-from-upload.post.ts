...
export default defineEventHandler(async (event) => {
  try {
    ...
    const elementSystem = [
      ...
      'Return plain text with clear headings and bullet points. Do NOT fabricate info. Leave fields blank if unknown.',
      language === 'zh' ? 'Language: Chinese (简体中文).' : 'Language: English.'
    ].join(' ')
    const elementUser = [
      ps_requirement || '',
      guidance_element || '',
    ...
    const outlineSystem = [
      ...
      'Output concise headings with 1-3 bullet points each. Do not write body paragraphs.',
      language === 'zh' ? 'Language: Chinese (简体中文).' : 'Language: English.'
    ].join(' ')
    ...
    const bodySystem = [
      ...
      'Do not invent facts; if information is missing, keep generic placeholders or skip that detail.',
      language === 'zh' ? 'Language: Chinese (简体中文).' : 'Language: English.'
    ].join(' ')
